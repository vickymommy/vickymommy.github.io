/* Vicky Mommy 網站 — 共用前台程式（多頁式）。
   新增文章：編輯 data/articles.json；社群/課程：編輯 data/extra.js。 */
const CAT_LABELS = { ai:"AI 學習應用", growth:"媽媽自我成長", parenting:"親子生活指南", travel:"親子旅遊", diy:"親子手作" };

function injectChrome(active){
  const nav = (id,href,label)=>'<a href="'+href+'" class="'+(active===id?'active':'')+'">'+label+'</a>';
...
function loadArticles(){
  const arts = [...(window.SITE_DATA ? window.SITE_DATA.articles : [])];
  arts.sort((a,b)=> (a.date<b.date?1:-1));
  return arts;
}
function cardHTML(a){
  const cover = a.cover
...
function renderFeatured(){
  const box=document.getElementById('featured-grid'); if(!box) return;
  box.innerHTML = loadArticles().slice(0,6).map(cardHTML).join('');
}
function renderListing(){
  const box=document.getElementById('listing-grid'); if(!box) return;
  const arts=loadArticles(); let cat='all', kw='';
  function apply(){
    const list=arts.filter(a=>(cat==='all'||a.category===cat)&&(kw===''||(a.title+a.excerpt+(a.tags||[]).join('')).toLowerCase().includes(kw.toLowerCase())));
    box.innerHTML=list.length?list.map(cardHTML).join(''):'<p style="grid-column:1/-1;text-align:center;color:var(--ink-soft);padding:40px">沒有符合的文章，換個關鍵字試試</p>';
    const c=document.getElementById('count'); if(c)c.textContent='共 '+list.length+' 篇文章';
  }
  document.querySelectorAll('.cat').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.cat').forEach(x=>x.classList.remove('active'));b.classList.add('active');cat=b.dataset.cat;apply();}));
  const s=document.getElementById('search-input'); if(s)s.addEventListener('input',e=>{kw=e.target.value;apply();});
  apply();
}
function renderArticle(){
  const box=document.getElementById('article-root'); if(!box) return;
  const id=new URLSearchParams(location.search).get('id');
  const a=loadArticles().find(x=>x.id===id);
  if(!a){box.innerHTML='<div class="pending">找不到這篇文章，<a href="articles.html">回到文章列表</a></div>';return;}
...
function renderCommunity(){
  const box=document.getElementById('community-root'); if(!box||!window.EXTRA) return;
  const E=window.EXTRA;
  box.innerHTML=
...
function renderIpas(){
  const box=document.getElementById('ipas-root'); if(!box) return;
  const E=window.EXTRA||{};
  const arts=loadArticles().filter(isIpas);
  box.innerHTML=
    '<section><div class="wrap">'+
...
function renderCourses(){
  const box=document.getElementById('courses-root'); if(!box||!window.EXTRA) return;
  const c=window.EXTRA.course, E=window.EXTRA;
  box.innerHTML=
    '<section><div class="wrap">'+
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  renderFeatured(); renderListing(); renderArticle();
  renderCommunity(); renderIpas(); renderCourses();
});
